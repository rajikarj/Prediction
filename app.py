...
task = st.radio("Choose your choice", ("Sentiment Classification", "Detection of tumor"))
...
if task == "Sentiment Classification":
    ...
    if new_review_text.strip():
        ...
        if model_option == "Perceptron":
            with open(r'D:\deep-learning\Assignment2\Perceptron.pkl', 'rb') as file:
                model = pickle.load(file)
        elif model_option == "Backpropagation":
            with open(r'D:\deep-learning\Assignment2\Backprop.pkl', 'rb') as file:
                model = pickle.load(file)
        elif model_option == "DNN":
            model = load_model(r'D:\deep-learning\Assignment2\DNN.keras')
        elif model_option == "RNN":
            model = load_model(r'D:\deep-learning\Assignment2\RNN.keras')
        elif model_option == "LSTM":
            model = load_model(r'D:\deep-learning\Assignment2\LSTM.keras')
...
elif task == "Detection of tumor":
    st.subheader(" Detection of tumor")
    uploaded_file = st.file_uploader("Choose a tumor image...", type=["jpg", "jpeg", "png"])

    if uploaded_file is not None:
        # Load the tumor detection model
        model = load_model(r'D:\deep-learning\Assignment2\CNN.keras')
